...
class Trainer(ParallelWorkerBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.daemon = daemon
        self.dev = dev
        self._client: AgentFlowClient | None = None  # Will be initialized in fit method
    ...
    def init(self, backend: Union[str, AgentFlowClient]) -> None:
    ...
    def client(self) -> AgentFlowClient:
        """Returns the AgentFlowClient instance."""
        if self._client is None:
            raise RuntimeError("AgentFlowClient has not been initialized. Call `init` first.")
        return self._client

    def _init_client(self, backend: Union[str, AgentFlowClient]) -> AgentFlowClient:
        if self._client is None:
            if isinstance(backend, AgentFlowClient):
                logger.info("Using provided AgentFlowClient instance.")
                self._client = backend
            else:
                logger.info(f"Initializing AgentFlowClient with endpoint: {backend}")
                if not isinstance(backend, str):
                    raise ValueError("backend must be a string URL or an AgentFlowClient instance.")
                ...
                self._client = AgentFlowClient(endpoint=backend)
        else:
            logger.warning("AgentFlowClient already initialized. Returning existing instance.")
        return self._client
    ...
    def kill_orphaned_processes() -> None:
        ...
        for proc in psutil.process_iter():
            ...
            if proc.name().startswith("AgentFlow-"):
                proc.kill()

    def fit(
        self,
        agent: LitAgent,
        backend: Union[str, AgentFlowClient],
        dev_backend: Union[str, AgentFlowClient, None] = None,
    ):
        if self.dev:
        ...
        try:
            if self.n_workers == 1:
                ...
                for i in range(self.n_workers):
                    process_name = f"AgentFlow-Worker-{i}"
